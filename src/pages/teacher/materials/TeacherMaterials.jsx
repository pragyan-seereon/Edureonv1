...
export default function TacherMaterials() {
  ...
  const submit = () => {
    if (!form.title || !form.url || form.klasses.length === 0) {
      toast.error("Title, URL and at least one class required");
      return;
    }
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Linked materials become visible to mapped classes.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  <Label>URL / File reference</Label>
                  <Input
                    value={form.url}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, url: e.target.value }))
                    }
                    placeholder="https://… or /files/foo.pdf"
                  />
                </div>
                <div className="space-y-1.5">
                  <Label>Map to classes</Label>
                  <div className="flex flex-wrap gap-1">
                    {sections.map((s) => (
                      <Badge
                        key={s.id}
                        variant={
                          form.klasses.includes(s.name) ? "default" : "outline"
                        }
                        className="cursor-pointer text-[11px]"
                        onClick={() =>
                          setForm((f) => ({
                            ...f,
                            klasses: f.klasses.includes(s.name)
                              ? f.klasses.filter((k) => k !== s.name)
                              : [...f.klasses, s.name],
                          }))
                        }
                      >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {m.subject} · {m.klasses.join(", ")} · {m.teacher}
                </div>
              </div>
