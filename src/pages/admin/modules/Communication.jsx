...
export default function Communication() {
  ...
  const drafts = useMemo(
    () => notices.filter((n) => n.status === "Draft"),
    [notices],
  );
  ...
  const send = (status) => {
    ...
    toast.success(
      status === "Published"
        ? "Notice published to recipients"
        : "Saved as draft",
    );
    reset();
  };
    ...
    <PageContainer>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <KpiCard
          ...
          value={published.reduce((a, n) => a + n.acks.length, 0)}
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="border-border/60">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex gap-1 pt-1 text-[10px] text-muted-foreground">
              <Smartphone className="h-3 w-3" />
              SMS <Bell className="h-3 w-3 ml-2" />
              Push <Mail className="h-3 w-3 ml-2" />
              Email — all bundled
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border-border/60">
            <CardContent className="p-0 divide-y">
              ...
              {published.map((n) => (
                <div
                  key={n.id}
                  className="p-4 hover:bg-muted/40 flex items-start gap-4"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {n.audience}
                      {n.targetClass ? ` · ${n.targetClass}` : ""} · {n.by} ·{" "}
                      {n.category}
                    </div>
              ...
              {archived.map((n) => (
                <div key={n.id} className="p-4 flex items-start gap-4">
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      noticesApi.update(n.id, { status: "Draft" });
