...
export default function AssignmentDetail() {
  ...
  const subs = useMemo(
    () => allSubs.filter((s) => s.assignmentId === id),
    [allSubs, id],
  );
  ...
  const avg =
    subs
      ...
      .reduce((acc, s) => acc + (s.marks || 0), 0) / Math.max(1, graded);
  const filtered = subs.filter((s) =>
    statusFilter === "All" ? true : s.status === statusFilter,
  );
  const acts = activity.filter(
    (ac) =>
      (ac.entity === "assignment" && ac.entityId === id) ||
      (ac.entity === "submission" && ac.entityId.includes(id)),
  );
  return (
    <PageContainer>
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                assignmentsApi.archive(a.id, !a.archived);
      ...
      <Tabs defaultValue="subs">
        <TabsList>
          ...
          <TabsTrigger value="comments">
            ...
            {
              comments.filter(
                (c) => c.entity === "assignment" && c.entityId === id,
              ).length
            }
            )
          </TabsTrigger>
                  ...
                  {filtered.map((s) => (
                    <TableRow key={s.id}>
                      ...
                      <TableCell className="tabular-nums">
                        {s.marks != null ? `${s.marks}/${a.maxMarks}` : "—"}
                      </TableCell>
                      ...
                      <TableCell>
                        {s.status === "Pending" ? (
                          <Button
                            ...
                            onClick={() => {
                              submissionsApi.submit(
                                ...
                                s.studentId,
                                s.studentName,
                                ["proxy-submission.pdf"],
                              );
                              toast.success("Submission simulated");
                            }}
                          >
                            Simulate Submit
                          </Button>
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() => {
                              submissionsApi.reopenGrading(s.id);
                              toast.success("Grading reopened");
                            }}
                          >
          ...
          <Card className="border-border/60">
            <CardHeader className="flex-row justify-between items-center space-y-0">
              ...
              <Button
                ...
                className="gradient-primary border-0"
                onClick={() => {
                  submissionsApi.bulkPublishGrades(a.id);
                  toast.success("All draft grades published");
                }}
              >
            ...
            <CardContent className="p-0 divide-y">
              {subs
                ...
                .map((s) => {
                  const draft =
                    draftGrades[s.id] ??
                    (s.draftGrade != null ? String(s.draftGrade) : "");
                  ...
                  return (
                    <div key={s.id} className="p-4 space-y-2">
                        ...
                        <div className="flex gap-2 items-center">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              if (isNaN(m)) return toast.error("Invalid marks");
                              submissionsApi.saveDraftGrade(s.id, m, fb);
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const m = Number(draft);
                              if (isNaN(m)) return toast.error("Invalid marks");
                              submissionsApi.publishGrade(s.id, m, fb);
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() => {
                              submissionsApi.returnForRevision(
                                s.id,
                                fb || "Please revise",
                              );
                      ...
                      <Textarea
                        ...
                        value={fb}
                        onChange={(e) =>
                          setDraftFb((p) => ({ ...p, [s.id]: e.target.value }))
                        }
                      />
                    </div>
                  );
          ...
          <Card className="border-border/60">
            <CardContent className="p-5">
              ...
              <Progress
                ...
                value={total ? (submitted / total) * 100 : 0}
              />
            </CardContent>
          </Card>
          <Card className="border-border/60">
            <CardContent className="p-5">
              ...
              <div className="text-2xl font-display font-semibold mt-1">
                {graded ? `${avg.toFixed(1)}/${a.maxMarks}` : "—"}
              </div>
            </CardContent>
          </Card>
          <Card className="border-border/60">
          ...
          <Card className="border-border/60">
            <CardContent className="p-4 space-y-3">
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    commentsApi.add("assignment", id, comment);
                    setComment("");
              ...
              <div className="space-y-2 divide-y">
                {comments
                  .filter((c) => c.entity === "assignment" && c.entityId === id)
                  .map((c) => (
                    <div key={c.id} className="pt-2">
                      <div className="text-xs text-muted-foreground">
                        {c.by} · {new Date(c.at).toLocaleString("en-IN")}
                      </div>
            ...
            <CardContent className="p-0 divide-y">
              ...
              {acts.map((ac) => (
                ...
                >
                  <span>{ac.action}</span>
                  <span className="text-xs text-muted-foreground">
                    {ac.by} · {new Date(ac.at).toLocaleString("en-IN")}
                  </span>
                </div>
              ))}
            </CardContent>
