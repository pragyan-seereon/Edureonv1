...
const SEED_ISSUES = [
  ...
  },
  {
    ...
    issuedOn: addDays(today(), -5),
    dueOn: addDays(today(), 9),
    feePerDay: 5,
    fineCollected: 0,
  },
  {
    ...
    issuedOn: addDays(today(), -30),
    dueOn: addDays(today(), -16),
    returnedOn: addDays(today(), -10),
    feePerDay: 5,
    fineCollected: 30,
  },
];
function calcFine(iss, refDate = today()) {
...
export default function LibraryPage() {
  ...
  const filtered = books.filter(
    (b) =>
      !query ||
      (b.isbn + b.title + b.author).toLowerCase().includes(query.toLowerCase()),
  );
  const handleReturn = (id) => {
    setIssues((p) =>
      p.map((i) => {
        ...
        toast.success(
          fine > 0 ? `Returned · Fine ₹${fine} collected` : "Returned",
        );
    ...
    <PageContainer>
      ...
      {issueOpen && (
        <IssueDialog
          ...
          onIssue={(iss) => {
            setIssues((p) => [iss, ...p]);
            toast.success(`Issued "${iss.title}" to ${iss.studentName}`);
            setIssueOpen(false);
          }}
        />
      )}
      ...
      <Tabs defaultValue="catalog">
          ...
          <Card className="border-border/60">
            <CardHeader className="flex-row items-center justify-between space-y-0">
              <div>
                ...
                <CardDescription>
                  {books.length} titles · click Issue Book to assign one to a
                  student
                </CardDescription>
              </div>
        ...
        <TabsContent value="history" className="mt-4">
          <IssuesTable
            issues={issues.filter((i) => i.returnedOn)}
...
function IssuesTable({ issues, onReturn }) {
  return (
    <Card className="border-border/60">
      <CardContent className="p-0">
        <Table>
            ...
            {issues.map((i) => {
              ...
              return (
                <TableRow key={i.id}>
                  ...
                  <TableCell
                    className={`tabular-nums text-sm ${fine > 0 && !i.returnedOn ? "text-destructive font-semibold" : ""}`}
                  >
                    {fine > 0 ? `₹${fine}` : "—"}
                  </TableCell>
                  <TableCell>
...
function IssueDialog({ books, availableMap, students, onClose, onIssue }) {
  ...
  const submit = () => {
    if (!book || !student) {
      toast.error("Pick a book and student");
      return;
    }
    onIssue({
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Set the return date and per-day late fee. Fines auto-calculate when
            overdue.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3 py-2">
          <div>
            <Label className="text-xs">
              Book ({availableBooks.length} available)
            </Label>
              ...
              <SelectContent className="max-h-72">
                {availableBooks.map((b) => (
                  <SelectItem key={b.isbn} value={b.isbn}>
                    {b.title} — {b.author} ({availableMap[b.isbn]} left)
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <Label className="text-xs">Return By</Label>
              <Input
                type="date"
                value={dueOn}
            ...
            <div>
              <Label className="text-xs">Late Fee (₹/day)</Label>
              <Input
                type="number"
                value={feePerDay}
