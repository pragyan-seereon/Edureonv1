...
import {
  ...
  getGatePassByUUID,
  getGatePasses,
  searchGatePassStudents,
  searchGatePassEmployees,
  returnGatePass,
} from "../../api/gatepass";
...
const mapPass = (row) => ({
  ...
  type: String(row.pass_type ?? "STUDENT")
    .toLowerCase()
    .replace(/^./, (letter) => letter.toUpperCase()),
  passCategory: row.type ?? "PERSONAL",
  ...
  status:
    String(row.status ?? "OUT").toUpperCase() === "OUT" ? "Out" : "Returned",
});

const passTime = (value) =>
  value
...
const passClassOrDepartment = (pass) =>
  pass.type === "Student"
    ? [
        pass.className,
        pass.sectionName && `Sec ${pass.sectionName}`,
        pass.rollNo !== "—" && `Roll ${pass.rollNo}`,
      ]
        .filter(Boolean)
        .join(" · ") || "—"
    : pass.department || "—";
...
function downloadPassPdf(pass) {
  ...
  const rows = [
    ...
    ["Contact", pass.contact, "Type", pass.passCategory],
...
export default function GatePassPage() {
  ...
  const [, setOriginalInTime] = useState("");
      ...
      .then((response) =>
        setSections(
          (response?.data ?? response ?? []).filter(
            (section) => section.class_uuid === form.classUuid,
          ),
        ),
      )
  ...
  const viewPass = async (pass) => {
    ...
    try {
      ...
      toast.error(
        requestError?.response?.data?.message ?? "Failed to load gate pass.",
      );
    } finally {
  ...
  const editPass = async (pass) => {
    ...
    try {
      ...
      setForm({
        ...
        expectedReturnAt: current.expectedReturnAt?.slice(11, 16) ?? "",
        ...
        accompaniedBy:
          current.accompaniedBy === "—" ? "" : current.accompaniedBy,
  ...
  const confirmDeletePass = async () => {
    ...
    try {
      ...
      toast.error(
        requestError?.response?.data?.message ?? "Failed to delete gate pass.",
      );
    } finally {
      setActionPassUuid(null);
    }
  ...
  const createPass = async () => {
    ...
    if (!editingPassUuid && !form.expectedReturnAt)
      errors.expectedReturnAt = "Expected return time is required.";
    ...
    try {
      if (editingPassUuid) {
        // The edit modal's Update action marks the issued pass as returned.
        await returnGatePass(editingPassUuid);
      } else {
        // Keep this object in exact POST /gate-pass schema order. Fields that
        // do not apply to the selected person type are deliberately null.
        const payload = {
          pass_type: form.passType,
          type: form.type,
          person_name: form.name.trim(),
          student_uuid: form.studentUuid || null,
          employee_uuid: form.employeeUuid || null,
          class_uuid: form.classUuid || null,
          section_uuid: form.sectionUuid || null,
          department_uuid: form.departmentUuid || null,
          out_time: timeToIso(form.outTime),
          expected_return_at: timeToIso(form.expectedReturnAt),
      ...
      toast.success(
        editingPassUuid ? "Gate pass updated." : "Gate pass generated.",
      );
      setCreateOpen(false);
      setForm(EMPTY_FORM);
      setEditingPassUuid(null);
      if (page === 1) loadPasses();
      else setPage(1);
    } catch (requestError) {
      toast.error(
        requestError?.response?.data?.message ??
          "Failed to generate gate pass.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <PageContainer>
        ...
        <CardContent className="p-0">
                    ...
                    <TableCell>
                      <div className="flex items-center justify-end gap-1">
                        <Button
                          ...
                          disabled={actionPassUuid === pass.id}
                          onClick={() => viewPass(pass)}
                        >
                          {actionPassUuid === pass.id ? (
          ...
          <div className="flex items-center justify-between border-t px-4 py-3">
            <span className="text-xs text-muted-foreground">
              Showing {passes.length ? (page - 1) * pageSize + 1 : 0}–
              {(page - 1) * pageSize + passes.length} of {total}
            </span>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {editingPassUuid ? "Edit Gate Pass" : "New Gate Pass"}
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
              ...
              <Select value={form.passType} onValueChange={changePassType}>
            ...
            <div className="relative">
              ...
              <Input
                ...
                onFocus={() => {
                  if (form.passType === "STUDENT" && studentResults.length > 0)
                    setShowStudentResults(true);
                  if (
                    form.passType === "EMPLOYEE" &&
                ...
                placeholder={
                  form.passType === "STUDENT"
                    ? "Type a student name"
                    : form.passType === "EMPLOYEE"
                      ? "Type an employee name"
                      : "Enter name"
                }
              />
              {form.passType === "STUDENT" &&
                (showStudentResults || searchingStudents) && (
                  <div className="absolute z-30 mt-1 max-h-52 w-full overflow-auto rounded-md border border-border bg-popover shadow-md">
                    ...
                    {studentResults.map((student) => (
                      <button
                        ...
                        className="w-full border-b border-border/60 px-3 py-2 text-left last:border-0 hover:bg-muted"
                      >
              ...
              {form.passType === "EMPLOYEE" &&
                (showEmployeeResults || searchingEmployees) && (
                  <div className="absolute z-30 mt-1 max-h-52 w-full overflow-auto rounded-md border border-border bg-popover shadow-md">
                    {searchingEmployees && (
                      <div className="px-3 py-2 text-xs text-muted-foreground">
                        Searching employees…
              ...
              <>
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(classUuid) =>
                      setForm({ ...form, classUuid, sectionUuid: "" })
                    }
                  >
                    <SelectTrigger className="mt-1">
                ...
                <div>
                  <Label>Section</Label>
                  <Select
                    value={form.sectionUuid}
                    onValueChange={(sectionUuid) =>
                      setForm({ ...form, sectionUuid })
                    }
                    disabled={!form.classUuid}
                  >
                    <SelectTrigger className="mt-1">
            ...
            {form.passType === "EMPLOYEE" && (
              <div>
                ...
                <Select
                  ...
                  onValueChange={(departmentUuid) =>
                    setForm((current) => ({ ...current, departmentUuid }))
                  }
                >
                  <SelectTrigger className="mt-1">
            ...
            <div>
              ...
              <Input
                className={
                  formErrors.outTime ? "mt-1 border-destructive" : "mt-1"
                }
                type="time"
                value={form.outTime}
                onChange={(event) => {
                  setForm({ ...form, outTime: event.target.value });
                  setFormErrors((current) => ({ ...current, outTime: "" }));
                }}
              />
              {formErrors.outTime && (
                <FieldError message={formErrors.outTime} />
              )}
            </div>
            <div>
              <Label>
              ...
              <Input
                className={
                  formErrors.expectedReturnAt
                    ? "mt-1 border-destructive"
                    : "mt-1"
                }
                type="time"
                value={form.expectedReturnAt}
                onChange={(event) => {
                  setForm({ ...form, expectedReturnAt: event.target.value });
              ...
              />
              {formErrors.expectedReturnAt && (
                <FieldError message={formErrors.expectedReturnAt} />
              )}
            </div>
            <div className="md:col-span-2">
              ...
              <Textarea
                className={
                  formErrors.purpose ? "mt-1 border-destructive" : "mt-1"
                }
                rows={2}
                value={form.purpose}
                onChange={(event) => {
                  setForm({ ...form, purpose: event.target.value });
                  setFormErrors((current) => ({ ...current, purpose: "" }));
                }}
              />
              {formErrors.purpose && (
      ...
      <Dialog
        open={Boolean(previewPass)}
        onOpenChange={(open) => !open && setPreviewPass(null)}
      >
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>Gate Pass · {previewPass?.number}</DialogTitle>
          </DialogHeader>
          {previewPass && (
            <div className="overflow-hidden rounded-xl border border-border/60">
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4 p-5 text-sm">
                ...
                <PassDetail
                  label="Class / Dept."
                  value={passClassOrDepartment(previewPass)}
                />
                <PassDetail label="Contact" value={previewPass.contact} />
                <PassDetail
                  label="Out Time"
                  value={passTime(previewPass.outTime)}
                />
                <PassDetail
                  label="In Time"
                  value={passTime(previewPass.inTime)}
                />
                <PassDetail
                  label="Purpose"
                  value={previewPass.purpose}
                  className="col-span-2"
                />
              </div>
              <div className="flex justify-between border-t border-border/60 px-5 py-4 text-xs text-muted-foreground">
          ...
          <DialogFooter>
            ...
            <Button
              className="gradient-primary border-0"
              onClick={() => downloadPassPdf(previewPass)}
            >
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          !open && !actionPassUuid && setDeleteTarget(null)
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Delete {deleteTarget?.number || "this gate pass"}? This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={Boolean(actionPassUuid)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90"
              disabled={Boolean(actionPassUuid)}
              onClick={(event) => {
