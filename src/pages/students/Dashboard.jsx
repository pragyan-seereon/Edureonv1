...
export default function Dashboard() {
  ...
  const name =
    user?.name?.split(" ")[0] ?? me?.name?.split(" ")[0] ?? "Student";
  const myAssignments = useMemo(
    () =>
      assignments.filter((a) => a.klass === klass && a.status === "Published"),
    [assignments, klass],
  );
  const mySubs = useMemo(
    () => subs.filter((s) => s.studentId === STUDENT_ID),
    [subs],
  );
  const pending = myAssignments.filter((a) => {
    const s = mySubs.find((x) => x.assignmentId === a.id);
    return !s || s.status === "Pending" || s.status === "Returned";
  }).length;
  const upExams = exams
    .filter(
      (e) =>
  ...
  const myNotices = notices
    ...
    .slice(0, 4);
  const myMarks = marks
    .filter((m) => m.studentId === STUDENT_ID && m.status === "Published")
    .slice(0, 4);
    ...
    <PageContainer>
        ...
        <Card className="lg:col-span-2 border-border/60">
          <CardHeader className="pb-2 flex-row items-center justify-between space-y-0">
            <div>
              ...
              <CardDescription>{myAssignments.length} active</CardDescription>
            </div>
          ...
          <CardContent className="space-y-3">
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-muted-foreground">Term 2 — paid</span>
                <span className="font-semibold">₹78,000 / ₹1,18,000</span>
              </div>
              <Progress value={66} className="h-2" />
            </div>
            <div className="text-xs text-muted-foreground">
              Balance ₹40,000 due 30 Nov 2025
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="border-border/60">
          ...
          <CardContent className="space-y-2">
            {events.map((e) => (
              <div key={e.id} className="p-2 border rounded-md text-sm">
