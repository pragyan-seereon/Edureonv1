...
const marks = Array.from({ length: daysInMonth }).map((_, i) => {
  const d = i + 1;
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        title="My Attendance"
        description="Daily attendance calendar with monthly trend."
      />
        ...
        <div className="mb-5 flex items-start gap-3 p-4 rounded-md bg-warning/10 border border-warning/30">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              As per CBSE norms, sustained attendance below 75% may affect board
              eligibility.
            </div>
          </div>
        </div>
          ...
          <CardHeader className="pb-2 flex-row items-center justify-between space-y-0">
            ...
            <div className="flex items-center gap-2 text-[10px]">
              {["P", "A", "L", "H"].map((m) => (
                <span
                  key={m}
                  className={`px-2 py-0.5 rounded border ${colorMap[m]}`}
                >
                  {m}
                </span>
              ))}
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-7 gap-1.5">
              ...
              {marks.map((m, i) => (
                <div
                  key={i}
                  className={`aspect-square rounded-md border text-xs font-semibold flex flex-col items-center justify-center ${colorMap[m]}`}
                >
                  <span>{i + 1}</span>
