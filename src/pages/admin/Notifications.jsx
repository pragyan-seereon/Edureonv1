    ...
    notices
      ...
      .forEach((n) =>
        out.push({
          ...
          title: n.title,
          desc: `${n.by} · ${n.audience}${n.targetClass ? " · " + n.targetClass : ""}`,
          ts: new Date(n.publishedAt || n.createdAt).getTime(),
          category: "Academic",
        }),
      );
  ...
  return (
    <PageContainer>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((n) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`h-10 w-10 rounded-md flex items-center justify-center shrink-0 ${n.tone}`}
                      >
      ...
      <div className="mt-3 text-[11px] text-muted-foreground flex items-center gap-2">
        ...
        Wired live to: notices, leave requests, attendance corrections,
        submissions, mark publishes, exams &amp; assignments.{" "}
        <Users className="h-3 w-3" />
        <CalendarCheck className="h-3 w-3" />
        <Link to="/communication" className="ml-auto underline">
          Compose notice →
        </Link>
      </div>
    </PageContainer>
