...
const invoices = [
  {
    id: "INV-2025-1182",
    tenant: "Delhi Public School — North",
    plan: "Enterprise",
    amt: "₹84,000",
    status: "Paid",
    date: "1 Nov 2025",
  ...
  {
    id: "INV-2025-1181",
    tenant: "St. Xavier's High School",
    plan: "Enterprise",
    amt: "₹92,000",
    status: "Paid",
    date: "1 Nov 2025",
  ...
  {
    id: "INV-2025-1180",
    tenant: "Greenfield International",
    plan: "Business",
    amt: "₹58,000",
    status: "Paid",
    date: "1 Nov 2025",
...
const flags = [
  {
    ...
    desc: "Generate scholastic + co-scholastic remarks with AI",
    enabled: true,
    scope: "Enterprise",
  },
  {
    name: "Face-Recognition Attendance",
    desc: "Camera-based marking · beta",
    enabled: false,
    scope: "Business+",
  },
  {
    name: "Parent Mobile App v3",
    desc: "Refreshed parent app · phased rollout",
    enabled: true,
    scope: "All tenants",
  },
  {
    name: "Stripe Connect Payouts",
    desc: "Direct payout to institute account",
    enabled: false,
    scope: "Enterprise",
  },
  {
    name: "Advanced BI Builder",
    desc: "Drag-drop dashboards",
    enabled: true,
    scope: "Business+",
  },
];
  ...
  return (
    <PageContainer>
        ...
        <TabsContent value="plans" className="mt-4 grid md:grid-cols-3 gap-4">
          {plans.map((p) => {
            ...
            return (
              <Card
                key={p.name}
                className={`relative ${isSubscribed ? "border-success shadow-md ring-2 ring-success/30" : p.pop ? "border-primary shadow-md" : "border-border/60"}`}
              >
                {isSubscribed && (
                ...
                {!isSubscribed && p.pop && (
                  <Badge className="absolute -top-2 left-4 gradient-primary border-0 text-primary-foreground">
                    Most Popular
                  </Badge>
                )}
                <CardHeader>
                ...
                <CardContent>
                  ...
                  {isSubscribed ? (
                    ...
                    <Button
                      className={`w-full ${p.pop ? "gradient-primary border-0" : ""}`}
                      variant={p.pop ? "default" : "outline"}
                      onClick={() =>
                        p.price === "Custom"
                          ? toast.success(
                              "Sales team will reach out within 24h",
                            )
                  ...
                  {invoices.map((i) => (
                    <TableRow key={i.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            i.status === "Paid" ? "default" : "destructive"
                          }
                        >
                          {i.status}
                        </Badge>
      ...
      <PaymentDialog
        ...
        plan={pay?.plan ?? ""}
        price={pay?.price ?? ""}
        onSuccess={(planName) => setSubscribed(planName)}
      />
    </PageContainer>
  );
}
function PaymentDialog({ open, onClose, plan, price, onSuccess }) {
  ...
  const reset = () => {
    setStage("method");
    setMethod("card");
    setForm({ card: "", name: "", expiry: "", cvv: "", upi: "", gst: "" });
  };
  const close = () => {
    reset();
    onClose();
  ...
  const pay = () => {
    if (
      method === "card" &&
      (!form.card || !form.name || !form.expiry || !form.cvv)
    ) {
        ...
        {stage === "success" ? (
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Secure payment · 256-bit TLS · PCI-DSS compliant
              </DialogDescription>
            </DialogHeader>
            ...
            {stage === "details" && (
              <div className="space-y-4">
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => setStage("method")}>
                    Back
                  </Button>
                  <Button className="gradient-primary border-0" onClick={pay}>
                    Pay {price}
                  </Button>
                </DialogFooter>
              </div>
...
function PayMethod({ id, current, icon, label, sub }) {
  const active = current === id;
  return (
    <label
      htmlFor={`pm-${id}`}
      className={`flex items-center gap-3 p-3 rounded-md border cursor-pointer transition ${active ? "border-primary bg-primary/5" : "hover:bg-muted/50"}`}
    >
