...
export default function Hostel() {
  ...
  const applyBulkAlloc = (assignments) => {
    setBlocks((p) =>
      p.map((b) => ({
        ...b,
        rooms: b.rooms.map((r) => {
          const adds = assignments
            .filter((a) => a.block === b.name && a.roomNo === r.no)
            .map((a) => a.alloc);
          return adds.length
            ? { ...r, allocations: [...r.allocations, ...adds] }
            : r;
        }),
      })),
    );
    toast.success(
      `${assignments.length} allocation${assignments.length > 1 ? "s" : ""} created across ${new Set(assignments.map((a) => a.roomNo)).size} room(s)`,
    );
        ...
        onSubmit={(d) =>
          setBlocks((p) => [
            ...p,
            {
              ...
              wardenCount: Number(d.wardenCount) || 1,
              rooms: [],
            },
          ])
        }
      ...
      {allocFor && (
        <AllocateDialog
          ...
          onAssignMany={(allocs) => {
            ...
            toast.success(
              `${allocs.length} student${allocs.length > 1 ? "s" : ""} allocated to ${ref.room.no}`,
            );
            setAllocFor(null);
          }}
        />
            ...
            <Card key={b.name} className="border-border/60">
              <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
                <div>
                  ...
                  <CardDescription>
                    Warden: {b.warden} · {b.wardenCount} warden
                    {b.wardenCount > 1 ? "s" : ""} · {b.rooms.length} rooms ·{" "}
          ...
          <Card className="border-border/60">
            <CardHeader className="flex-row items-center justify-between space-y-0 pb-3">
              <div>
                ...
                <CardDescription>
                  {allAllocs.length} student{allAllocs.length !== 1 ? "s" : ""}{" "}
                  allocated · {totalCapacity - occupied} vacant beds
                </CardDescription>
              </div>
              <Button
                size="sm"
...
function AllocateDialog({ block, room, students, onClose, onAssignMany }) {
  ...
  const batches = Array.from(new Set(students.map((s) => batchOf(s)))).filter(
    (b) => b !== "—",
  );
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="font-display">
            Allocate Students — {block} / {room.no}
          </DialogTitle>
          <DialogDescription>
            {room.type} · {beds.length} free bed{beds.length !== 1 ? "s" : ""} ·
            Selected {picked.size}/{maxPick}. Academic Year is informational.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="text-xs text-muted-foreground self-end">
              Beds auto-assigned: {beds.slice(0, picked.size).join(", ") || "—"}
            </div>
          </div>
        </div>
        <DialogFooter>
          ...
          <Button
            ...
            onClick={onConfirm}
          >
            Allocate {picked.size > 0 ? `(${picked.size})` : ""}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function BulkAllocateDialog({ blocks, students, onClose, onAllocate }) {
  ...
  const roomOptions = blocks.flatMap((b) =>
    b.rooms
      .map((r) => ({
        ...
        type: r.type,
        capacity: r.capacity,
        used: r.allocations.map((a) => a.bed),
        free: r.capacity - r.allocations.length,
      }))
      .filter((r) => r.free > 0),
  );

  const roomKey = (block, no) => `${block}|${no}`;
  ...
  const toggleStudent = (id) => {
    ...
    if (next.has(id)) next.delete(id);
      ...
      toast.error(`Only ${totalFreeBeds} free bed(s) in selected rooms`);
      return;
    }
    setPicked(next);
  };
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Filter students by class & section, bulk-select them, then pick one
            or more rooms. Students are distributed across the selected rooms
            automatically. Selected {picked.size} student(s) · {totalFreeBeds}{" "}
            free bed(s).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid lg:grid-cols-2 gap-4 py-2">
          ...
          <div className="space-y-3">
              ...
              {roomOptions.map((r) => {
                ...
                return (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={rooms.has(key)}
                      onChange={() => toggleRoom(key)}
                    />
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="text-xs text-muted-foreground self-end">
                {selectedRooms.length} room(s) · {totalFreeBeds} bed(s) selected
              </div>
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button
            className="gradient-primary border-0"
            disabled={picked.size === 0 || selectedRooms.length === 0}
            onClick={onConfirm}
          >
