...
// --- Static demo data (swap for real auth/store data as needed) ---
...
const initialWallet = [
  {
    ...
    type: "Credit",
    amount: 6000,
    reason: "Overpayment adjustment — Term 2 tuition",
    date: "2025-09-29T10:15:00",
  },
  {
    id: "wal-2",
    studentId: "stu-101",
    type: "Credit",
    amount: 2500,
    reason: "Stream change credit — Science to Commerce",
  ...
  },
  {
    ...
    type: "Adjustment",
    amount: 3000,
    reason: "Adjusted against Term 3 tuition",
...
const initialRequests = [
  {
    ...
    toClass: "8",
    toSection: "A",
    reason: "Closer friend group is in Section A",
    status: "Pending",
  },
];
...
export default function StudentWallet() {
  const [entries, setEntries] = useState(initialWallet);
  const [myReqs, setMyReqs] = useState(initialRequests);

  const balance = entries.reduce(
    (s, w) => s + (w.type === "Credit" ? w.amount : -w.amount),
    0
  );
  ...
  const requestRefund = () => {
    if (refundAmt <= 0 || refundAmt > balance) {
    ...
    toast.success(`Refund of ${inr(refundAmt)} requested`);
    setRefundOpen(false);
    setRefundAmt(0);
    setAcct("");
    setIfsc("");
        ...
        <Card className="border-border/60 lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <div>
              ...
              <CardDescription>
                All credits, debits and refund requests on your account.
              </CardDescription>
            </div>
            <Button
              size="sm"
              ...
              <TableBody>
                ...
                {entries.map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell>
                      <Badge variant={e.type === "Credit" ? "default" : "outline"}>
                        {e.type}
                      </Badge>
                    </TableCell>
              ...
              {myReqs.map((r) => (
                <div key={r.id} className="rounded-md border p-2 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    <span>
                      {r.fromClass}-{r.fromSection} → {r.toClass}-{r.toSection}
                    </span>
                    <Badge
                      variant={
                        r.status === "Approved"
                  ...
                  <div className="text-muted-foreground truncate" title={r.reason}>
                    {r.reason}
                  </div>
                </div>
              ))}
      ...
      <Dialog open={refundOpen} onOpenChange={setRefundOpen}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button className="gradient-primary border-0" onClick={requestRefund}>
              Submit
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                rows={4}
                placeholder="Why do you want to change your section?"
              />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button className="gradient-primary border-0" onClick={submitReq}>
              Submit Request
            </Button>
          </DialogFooter>
        </DialogContent>
