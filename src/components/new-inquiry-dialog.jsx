...
//   Odisha: [
//     "Bhubaneswar", "Cuttack", "Puri", "Berhampur", "Rourkela",
//     "Sambalpur", "Balasore", "Bhadrak", "Jajpur", "Baripada",
//   ],
...
//   Uttar_Pradesh: ["Lucknow", "Kanpur", "Varanasi", "Agra", "Prayagraj", "Noida"],
...
// const STATE_OPTIONS = Object.keys(STATE_CITY_MAP).map((state) =>
//   state.replaceAll("_", " ")
// );

// const getCitiesForState = (state) =>
//   STATE_CITY_MAP[state?.replaceAll(" ", "_")] || [];
...
//     // Keep the inquiry form session synchronized with the global active session.
//   // The user should not manually choose a different session inside this dialog.
//   useEffect(() => {
//     if (!open) return;
...
//       // Session year always comes from the global session store.
//       // Do not allow a stale/manual value from the dialog to override it.
//       Object.entries({
//         ...d,
//         session_year: sessionYear,
//       }).forEach(([key, value]) => {
...
//               <Input
//                 value={sessionYear || d.session_year || ""}
//                 readOnly
//                 disabled
...
//                   <SelectValue
//                     placeholder={
//                       !d.state ? "Select state first" : "Select city"
//                     }
//                   />
//                 </SelectTrigger>
//                 <SelectContent>
//                   {getCitiesForState(d.state).map((city) => (
...
export function NewInquiryDialog({ trigger, onCreate }) {
  ...
  useEffect(() => {
    ...
    const fetchAdmissionSources = async () => {
      try {
        const response = await getAdmissionSources();
        const data = normalizeArrayResponse(response);

        console.log("Admission sources:", data);

        if (!cancelled) {
          setSources(data);
        }
      } catch (error) {
        console.error("Admission sources error:", error);
        if (!cancelled) {
          setSources([]);
          toast.error("Failed to load admission sources");
        }
      }
    };
    ...
    const fetchEmployees = async () => {
      setLoadingEmployees(true);
      try {
        const response = await getEmployees({ is_active: true });
        const data = normalizeArrayResponse(response);
        ...
        if (!cancelled) {
          setEmployees([]);
          // Employee lookup is optional and must not block the inquiry form.
        }
      } finally {
        if (!cancelled) {
          setLoadingEmployees(false);
    ...
    // Run independently. A failure in classes/employees must not prevent
    // admission sources from appearing in the Admission Source dropdown.
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <TabsContent value="personal" className="grid sm:grid-cols-2 gap-3 mt-4">
            ...
            <F label="Admission Source">
              <Select
                value={d.source_id ? String(d.source_id) : ""}
                onValueChange={(v) => set("source_id", v)}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Select Source" />
