            ...
            <Table>
              ...
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell>
                      <Select value={r.category} onValueChange={(v) => patchRow(r.id, { category: v })}>
                    ...
                    <TableCell>
                      <Select value={r.diff} onValueChange={(v) => patchRow(r.id, { diff: v })}>
                        <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
...
export function TemplatesTab() {
  ...
  const [open, setOpen] = useState(false);
  const [edit, setEdit] = useState(null);
  const [f, setF] = useState({ name: "", subject: "Math", category: "MCQ", diff: "Easy", marks: "1", body: "" });

  const openNew = () => { setEdit(null); setF({ name: "", subject: "Math", category: "MCQ", diff: "Easy", marks: "1", body: "" }); setOpen(true); };
  const openEdit = (t) => { setEdit(t); setF({ name: t.name, subject: t.subject, category: t.category, diff: t.diff, marks: String(t.marks), body: t.body }); setOpen(true); };
  const save = () => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1"><Label className="text-xs">Category</Label><Select value={f.category} onValueChange={(v) => setF({ ...f, category: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{QUESTION_CATEGORIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
            <div className="space-y-1"><Label className="text-xs">Difficulty</Label><Select value={f.diff} onValueChange={(v) => setF({ ...f, diff: v })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{DIFFS.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
